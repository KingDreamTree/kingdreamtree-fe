...
type InbodyUnreadableScreenProps = { onConfirm: () => void; onSkip: () => void; onPrevious: () => void }

export function InbodyUnreadableScreen({ onConfirm, onSkip, onPrevious }: InbodyUnreadableScreenProps) {
  return <FixedStepFrame label="인바디-결과지 읽지 못함"><div className="inbody-unreadable-page">
    ...
    <section className="inbody-unreadable-guide" role="status"><span><img src={inbodyUnreadableInfo} alt="" /></span><div><strong>결과지를 읽지 못했어요.</strong><p>직접 입력하시거나 건너뛸 수 있습니다.</p></div></section>
    ...
    <button className="inbody-unreadable-skip" type="button" onClick={onSkip}>건너뛰기</button><button className="inbody-unreadable-confirm" type="button" onClick={onConfirm}>확인 완료</button><button className="inbody-unreadable-previous" type="button" onClick={onPrevious}><img src={inbodyPreviousArrow} alt="" />이전 단계</button><p className="inbody-unreadable-note">* 이 값으로 진행 및 확인 기록이 저장됩니다.</p>
  </div></FixedStepFrame>
}
