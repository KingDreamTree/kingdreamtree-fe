...
export const MESSAGES = {
  ...
  // evaluate() 가 내는 사유가 아니다 — 갤러리 경로에서 mirrorSuspected() 가
  // true 일 때 프론트가 직접 띄운다 (아래 거울 절 참고).
  MIRROR_SUSPECTED: "좌우가 반전된 사진 같아요. '거울로 찍은 사진'에 체크하고 다시 올려주세요.",
  OK: "좋습니다. 그대로 유지해주세요.",
};
...
//
// 규칙은 화면당 하나다. **판정 방향 = 프리뷰(사진) 방향.**
//
//   실시간 촬영(거울 프리뷰):  evaluate(mirrorLandmarks(ref), user, ...)
//   갤러리 업로드(프리뷰 없음): evaluate(ref, user, ...)                — 기존 그대로
//   갤러리 + is_mirrored 체크:  evaluate(mirrorLandmarks(ref), user, ...) — 실시간과 같다
//     사용자가 "이 사진은 거울 방향"이라고 신고한 것이므로 같은 규칙의 적용이다.
//     서버가 저장 직전에 사진·랜드마크를 되돌리므로 저장 방향은 원본으로 균일하다.
//
...
/**
 * 갤러리 업로드가 **자세 사유로** 거부됐을 때, 좌우 반전이면 설명되는가.
 * true 면 MESSAGES.MIRROR_SUSPECTED 를 띄우고 is_mirrored 체크를 유도한다.
 *
 * 두 경우가 여기로 들어온다 — 구분할 수 없고, 처방이 같아 구분할 필요도 없다.
 *   1. 사진 자체가 반전 저장돼 있다 (전면 카메라 "미리보기대로 저장", 거울 촬영)
 *   2. 사용자가 레퍼런스를 화면에 **보이는 대로**(거울처럼) 따라 하고 원본으로 찍었다
 *
 * 실측(2026-08-17, 리허설 신고 재현) — 비대칭 레퍼런스(한 손 공·한 손 허리)에서
 * 거울 모방은 팔 세그먼트가 hard_tol_deg 를 넘어 **정확히 0%** 가 된다. 수직에
 * 가까운 다리는 반전해도 각도가 안 변하므로, 대칭 자세에서는 이 문제가 안 보인다 —
 * 리허설 전까지 아무도 못 본 이유다.
 *
 * ⚠️ **판정을 바꾸지 않는다. 힌트 문구를 띄울지만 정한다.** "양방향 계산해서 좋은
 *    쪽 채택"이 기각된 이유(저장 방향이 사용자마다 뒤섞임)는 그대로 유효하다.
 *    방향이 실제로 바뀌는 건 사용자가 체크박스로 신고한 재업로드뿐이고, 그때는
 *    서버가 사진을 되돌려 방향이 균일해진다.
 *
 * ⚠️ 자세 사유(POSE — JOINT_TOO_FAR 포함)일 때만 본다. 관절 부족·부위 미커버·
 *    프레이밍·여러 명은 좌우와 무관하다 — 그때 이 힌트를 띄우면 사용자가
 *    엉뚱한 걸 고치게 된다.
 */
export function mirrorSuspected(ref, user, criteria, opts = {}) {
  const straight = evaluate(ref, user, criteria, opts);
  if (straight.blockReason !== "POSE") return false;
  return !evaluate(mirrorLandmarks(ref), user, criteria, opts).blocked;
}
