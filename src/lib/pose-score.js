...
// --------------------------------------------------------------------------
// 거울 처리 — 실시간 촬영 화면 전용 (2026-08-18 개정)
//
// 셀피 미리보기는 거울(좌우반전)인데 레퍼런스는 원본으로 보여준다. 그래서
// 사용자가 화면에 **보이는 대로** 따라 하면 해부학적으로는 좌우가 반전된
// 자세가 된다 — 그게 자연스러운 행동이므로 그 방향이 정답이어야 한다.
//
// 규칙은 하나다. **판정 방향 = 저장 방향 = 화면에 보이는 방향.**
//
//   실시간 촬영: 판정은 evaluate(mirrorLandmarks(ref), user, ...) 로 하고,
//               셔터에서 **사진과 랜드마크를 함께 반전해** 저장한다.
//               → 저장본이 레퍼런스와 같은 방향이 되어, 저장 이후 처리가
//                 갤러리 업로드와 완전히 같아진다 (교차 짝짓기 불필요).
//   갤러리 업로드: 프리뷰가 없으므로 evaluate(ref, user, ...) — 사진 그대로.
//
// ⚠️ 사진만 뒤집고 좌표는 원본으로 보내면(또는 반대) **에러 없이** 좌우 진단이
//    통째로 어긋난다. 둘은 반드시 세트로 움직인다.
// ⚠️ "양방향 다 계산해서 좋은 쪽 채택"은 기각됐다 — 판정 기준이 사용자마다
//    달라지고, 저장 방향이 뒤섞여 downstream(좌우 부위 진단)이 어느 방향인지
//    알 수 없게 된다. 한 규칙이면 방향이 균일하다.
// ⚠️ **레퍼런스 이미지를 화면에서 뒤집는 방식은 기각됐다.** 사진 속 배경
//    글자가 뒤집혀 보이는 표시 품질 문제. 표시는 원본, 판정만 거울 기준.
// ⚠️ 저장본의 "왼팔"은 실제 몸의 오른팔이다(거울이므로). 인바디 좌우 실측을
//    진단에 섞는 경로와는 어긋나므로 그쪽 처리 방침은 별도 합의가 필요하다.
// --------------------------------------------------------------------------
 ...
 *
 * 실시간 촬영에서 두 곳에 쓴다 (2026-08-18):
 *   1. 판정 기준 — 레퍼런스에 적용해 거울 프리뷰 기준으로 채점
 *   2. 저장 — 셔터에서 사용자 랜드마크에 적용, **캔버스 반전과 세트로**
 *
 ...
 * ⚠️ **사용자 좌표에 적용할 때는 사진도 같이 반전해야 한다.** 한쪽만 뒤집으면
 *    이름표가 사진 속 몸과 어긋나 왼팔 진단이 오른팔에 붙는데 에러는 안 난다.
 */
...
/** 갤러리 사진이 좌우 반전일 가능성을 안내하기 위한 판정이다. */
