...
/**
 * 정방향과 거울 방향 중 **더 잘 맞는 쪽**으로 판정한다. 실시간 촬영 화면은
 * evaluate 대신 이걸 쓴다.
 *
 * 선택 규칙: 통과(pass)하는 쪽 우선, 둘 다 같으면 자세 점수가 높은 쪽.
 * 결과에 mirroredReference 가 붙는다 — 거울 방향으로 매칭됐는지 표시용.
 *
 * ⚠️ **업로드 값은 채택된 쪽의 것을 그대로 보낸다.** 서버는 숫자만 받아
 *    임계값과 비교하므로 어느 방향으로 매칭됐는지는 서버 판정에 영향이 없다.
 * ⚠️ 사진·랜드마크는 **카메라 원본 그대로** 올린다. 거울 방향으로 매칭됐다고
 *    이미지나 좌표를 뒤집어 보내면 안 된다 — 이름표가 실제 몸과 어긋나서
 *    왼팔 진단이 오른팔에 붙는다 (is_mirrored 경로는 이 용도가 아니다).
 */
export function evaluateEitherWay(ref, user, criteria, opts = {}) {
  const straight = evaluate(ref, user, criteria, opts);
  const mirrored = evaluate(mirrorLandmarks(ref), user, criteria, opts);

  let best;
  if (straight.pass !== mirrored.pass) best = straight.pass ? straight : mirrored;
  else best = mirrored.pose_similarity > straight.pose_similarity ? mirrored : straight;

  return { ...best, mirroredReference: best === mirrored };
}
