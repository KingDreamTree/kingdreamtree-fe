...
- `user_id` 는 `POST /users` (바디 없음) 로 발급받아 로컬에 저장하되, **홈 진입
  (온보딩 [시작하기])마다 옛 값을 지우고 새로 발급**합니다. 세션이 도는 동안만
  유지하는 값입니다.

  ⚠️ **«최초 1회 발급 후 영구 보관» 은 폐기된 정책입니다** (#169). 옛 `user_id` 가
     남아 있으면 `POST /sessions` 가 409 로 옛 세션을 되돌려주고, 그 세션의 운동
     기록을 그대로 이어받아 **처음 들어온 사용자에게 «Day 4» 게이지가 뜹니다.**
     돌아왔을 때 기록이 없는 것이 **의도된 동작**입니다.
...
- [ ] `X-User-Id` 를 모든 요청에 싣는가 · **홈 진입마다 `user_id` 를 새로 발급**하는가 (#169)
