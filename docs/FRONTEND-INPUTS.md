...
| 무엇 | 왜 | 언제 |
|---|---|---|
| **프론트 배포 주소(오리진)** — `https://도메인[:포트]` | 서버 `.env` 의 `CORS_ORIGINS` 에 등록해야 합니다. ⚠️ **프로덕션은 이미 잠겨 있습니다.** 허용 목록에 없는 오리진에서는 모든 호출이 "Failed to fetch"로 죽습니다 — 서버는 멀쩡한데 화면에서만 실패하므로 원인을 찾는 데 오래 걸립니다. 2026-08-17 기준 등록된 것: `https://www.refit.live` · `https://kingdreamtree-fe.vercel.app` · `http://localhost:5173`(+5174·5175). ⚠️ **Vercel 프리뷰 URL은 브랜치마다 주소가 달라 별도 등록이 필요합니다** — 프리뷰로 테스트하려면 그 도메인을 먼저 알려주세요. 목록은 서버 `.env` 가 원본이라 이 문서가 뒤처질 수 있습니다 | **배포 주소 확정 즉시.** 바뀌면 그때마다 다시 |
